	...
	boundingbox_t bbox, std::vector<line_float_t> &lines)
{
	...
	case HOUGH_LINE_PROBABILISTIC:
		hough_line_probabilistic(src, sub_src->xsize, sub_src->ysize,
			(double)hough_rho, (double)hough_theta, hough_thresh,
			(double)min_theta_linelength, (double)max_theta_gap,
			lines);
...
//两条直线夹角小于1°的认为是平行
LINE_STATE HoughLineDetector::judge_two_lines(const line_param_t& line_a,const line_param_t& line_b){

	if(std::abs(line_a.angle-line_b.angle) < angle_threshold_){
		float two_line_distance_ = two_line_distance(line_a,line_b);
		// printf("two_line_distance %f\n",two_line_distance_);lines_need_merge(line_a,line_b) &&
		if( std::abs(line_a.b - line_b.b) < intercept_threshold_ && two_line_distance_<two_line_distance_threshold_ ){
			return LINES_COLINEAR;
		}
		else
			return LINES_PARALLEL;
	}
	else
		return LINES_NON_PARALLEL;
}

bool HoughLineDetector::detect(unsigned char* img,int width, int height,boundingbox_t bbox){
	if(img ==NULL)
		return false;
	origin_point_.x = height /2;
	origin_point_.y = width /2;
	pixel_float_t scale = {scale_x_,scale_y_};
	double sigma = 0.2;
	image_int8u_p _src = new_image_int8u_ptr(width,height,img);
	image_int8u_p sub_src = gaussian_sampler_byte_bbox(_src, bbox, scale, sigma);

	hough_line_probabilistic(sub_src->data, width, height,
			delta_rho_, delta_theta_, min_vote_,
			min_length_, max_gap_in_line_,
			lines_);
	free_image_int8u(sub_src);
	delete[]_src;
}
// 进来的两条直线都是平行的,带入四个点坐标
line_param_t HoughLineDetector::mergeLines(const line_param_t& line_a,const line_param_t& line_b){
	line_param_t result_line_param;

	float angle = (line_a.angle + line_b.angle)*0.5;
	float b = (line_a.b +line_b.b)*0.5;

	// result_line_param.end_point = compare_points(line_a.end_point,line_b.end_point)?line_b.end_point:line_a.end_point;
	// result_line_param.start_point = compare_points(line_a.start_point,line_b.start_point)?line_a.start_point:line_b.start_point;

	// result_line_param.end_point = line_a.start_point;
	// result_line_param.start_point = line_b.end_point;


	// 针对两条垂直于x轴
	if(abs(angle) < 45){
		result_line_param.start_point = line_a.start_point.x > line_a.end_point.x ? line_a.end_point:line_a.start_point;
		result_line_param.start_point = result_line_param.start_point.x > line_b.end_point.x ? line_b.end_point : result_line_param.start_point;
		result_line_param.start_point = result_line_param.start_point.x > line_b.start_point.x ? line_b.start_point : result_line_param.start_point;

		result_line_param.end_point = line_a.start_point.x < line_a.end_point.x ? line_a.end_point:line_a.start_point;
		result_line_param.end_point = result_line_param.end_point.x < line_b.end_point.x ? line_b.end_point : result_line_param.end_point;
		result_line_param.end_point = result_line_param.end_point.x < line_b.start_point.x ? line_b.start_point : result_line_param.end_point;

	}else{
		result_line_param.start_point = line_a.start_point.y > line_a.end_point.y ? line_a.end_point:line_a.start_point;
		result_line_param.start_point = result_line_param.start_point.y > line_b.end_point.y ? line_b.end_point : result_line_param.start_point;
		result_line_param.start_point = result_line_param.start_point.y > line_b.start_point.y ? line_b.start_point : result_line_param.start_point;

		result_line_param.end_point = line_a.start_point.y < line_a.end_point.y ? line_a.end_point:line_a.start_point;
		result_line_param.end_point = result_line_param.end_point.y < line_b.end_point.y ? line_b.end_point : result_line_param.end_point;
		result_line_param.end_point = result_line_param.end_point.y < line_b.start_point.y ? line_b.start_point : result_line_param.end_point;
	}
	result_line_param.angle = angle;
	result_line_param.b = b;
	return result_line_param;		
}

bool HoughLineDetector::handlLines(std::vector<line_param_t>& merged_lines){

	if(getLineParam()){
		int lines_size = line_params_.size();
		//两层for循环合并线段
		// assert(lines_size >= 2);
		...
		for(int i=0;i<lines_size-1;++i){
			// if(line_params_[i]b==0)
			// 	printf("b==0,angle %f\n",line_params_[i]);
			for(int j=i+1;j<lines_size;++j){
				if(!line_params_[i].is_merged && !line_params_[j].is_merged){
					if(judge_two_lines(line_params_[i],line_params_[j]) == LINES_COLINEAR){
						line_params_[i] = mergeLines(line_params_[i],line_params_[j]);
						line_params_[j].is_merged = true;
					}
				}else
					continue;
			}
		...
		for(int i=0;i<lines_size;++i){
			if(!line_params_[i].is_merged ){
				merged_lines.push_back(line_params_[i]);
			}
		}

		return true;
		// for(int i=0;i<merged_lines_size-1;++i){
		// 	for(int j=i+1;j<merged_lines_size;++j){
		// 		if(judge_two_lines(line_params_[i],line_params_[j]) == LINES_PARALLEL){
...
bool HoughLineDetector::getLineParam(){
	if(lines_.empty())
		return false;
	line_params_.clear();
	std::vector<line_float_t>::const_iterator it = lines_.begin();
    while (it!=lines_.end())
    {	
		
		line_param_t line_param;
		if(it->startx == it->endx ||it->starty == it->endy){
			it++;
			continue;
			// return false;
		}
		...
		float dx = it->startx - it->endx;
		float dy = it->starty - it->endy;
		if(dx == 0)
			printf("get vertical line\n");
		float radian = atanf(dy/dx);
		if(dx == 0)
			printf("get vertical line %f\n",radian);
		float angle = radian/PI*180;
		float b = 0;
		if(abs(angle - 90)<0.05){
			b = it->startx;
		}else{
			b = it->starty - tanf(radian) * it->startx;
		}

		line_param.angle = angle;
		line_param.b = b;
		
		point2f startPoint,endPoint;
		startPoint.x = it->startx;
		startPoint.y = it->starty;
		endPoint.x = it->endx;
		endPoint.y = it->endy;
		// float line_dis = two_point_distance(startPoint,endPoint);
		// // printf("line_dist %f\n",line_dis);
		// if( line_dis < 10)
		// 	continue;
		if(compare_points(startPoint,endPoint)){
			line_param.start_point = endPoint;
			line_param.end_point = startPoint;
		}else{
			line_param.start_point = startPoint;
			line_param.end_point = endPoint;
		}

		line_params_.push_back(line_param);
		it++;
    }
	std::sort(line_params_.begin(),line_params_.end(),&compare_lines);
	return true;
}
bool HoughLineDetector::judgeHallway(const std::vector<line_param_t>& merged_lines){
	int merged_lines_size = merged_lines.size();

	if(judge_two_lines(merged_lines[0],merged_lines[1]) == LINES_PARALLEL 
		&& origin_between_lines(merged_lines[0],merged_lines[1]) ){
		printf("Hall way detected\n");
		return true;
	}
	printf("not hall way!\n");
	return false;
